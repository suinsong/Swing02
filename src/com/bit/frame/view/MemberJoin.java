...
public class MemberJoin extends JFrame {
	...
	class JoinAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (rBtn1.isSelected())
				dto.setUserSex("남자");// 남자가 선택
			if (rBtn2.isSelected())
				dto.setUserSex("여자");// 여자가 선택
			dto.setUserTel(txtTel.getText());

			int ret = memberDAO.insert(dto);
			if (ret > 0) {
				JOptionPane.showMessageDialog(jPan1, Integer.toString(ret) + " 개수가 정상적으로 추가 되었습니다");
			} else {
				JOptionPane.showMessageDialog(jPan1, " 데이터 추가에 실패 했습니다.");
			}
